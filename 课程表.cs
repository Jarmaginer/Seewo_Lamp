...
namespace Seewo_Lamp
{
    class 课程表 : Formium
    {
        ...
        // 指定启动 Url
#if DEBUG
    public override string StartUrl { get; } = "http://localhost:9001/root/sc.html"; // 如果在 Debug 模式，那么请自行使用静态服务器来运行页面。
                                                                                     // 我使用的live-server，启动方法：
                                                                                     // live-server --port=9001
#else
        public override string StartUrl { get; } = "https://jar-wei.github.io/Seewo_Lamp_Service/sc.html"; // 如果是 Release 那么就自动把页面作为内嵌资源打包进程序集。
